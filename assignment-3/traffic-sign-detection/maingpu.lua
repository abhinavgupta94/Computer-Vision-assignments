...
function transformJittered(img)
    f = tnt.transform.compose{
        [1] = rotatejitter,
        [2] = translatejitter,
        [3] = colorscale
...
function getJitteredSample(dataset, idx)
    ...
    transformed = transformJittered(image.load(DATA_PATH .. '/train_images/'..file))
...
jittdataset = tnt.ListDataset{
    list = torch.range(1, trainData:size(1)*2):long(),
    load = function(idx)
        return {
            input = getJitteredSample(trainData, math.ceil(idx/2)),
            target = getTrainLabel(trainData, math.ceil(idx/2))
        }
    end
}
